...
class HttpTaskServerHistoryPrioritizedTest extends BaseHttpTest {
  ...
  void history_and_prioritized_ok() throws Exception {
    ...
    assertEquals(201, post(baseUrl, "/tasks", gson.toJson(t2)).statusCode());
    ...
    int t1Id = idByIndex(get(baseUrl, "/tasks").body(), 0);
    int t2Id = idByIndex(get(baseUrl, "/tasks").body(), 1);
    int sId = firstIdFromArray(get(baseUrl, "/subtasks").body());

    assertEquals(200, get(baseUrl, "/tasks/" + t1Id).statusCode());
    assertEquals(200, get(baseUrl, "/epics/" + epicId).statusCode());
    assertEquals(200, get(baseUrl, "/subtasks/" + sId).statusCode());

    // history should be non-empty (contains 3 entries)
    JsonArray hist = JsonParser.parseString(get(baseUrl, "/history").body()).getAsJsonArray();
    assertTrue(hist.size() >= 3);

    // prioritized — строго по возрастанию startTime: 10:00, 11:00, 12:00
    JsonArray pr = JsonParser.parseString(get(baseUrl, "/prioritized").body()).getAsJsonArray();
  ...
  /* helpers (не дублируют TestHttpUtils) */
  ...
  private static int idByIndex(String json, int idx) {
