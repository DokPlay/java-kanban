...
class HttpTaskServerSubtasksTest extends BaseHttpTest {
    ...
    void subtask_crud_and_overlap406() throws Exception {
        ...
        // create subtask
        Subtask s = new Subtask("S1", "", Status.NEW,
                Duration.ofMinutes(10), LocalDateTime.of(2035, 1, 1, 12, 0), epicId);
        ...
        JsonArray arr = JsonParser.parseString(get("/epics/" + epicId + "/subtasks").body()).getAsJsonArray();
        assertEquals(1, arr.size());

        // overlap by creating another subtask at same time for same epic
        Subtask clash = new Subtask("S2", "", Status.NEW,
                Duration.ofMinutes(15), LocalDateTime.of(2035, 1, 1, 12, 5), epicId);
