...
class PrioritizedUniquenessTest extends BaseHttpTest {
    ...
    @BeforeEach
    void setUp() throws IOException { setUpBase(); }

    @AfterEach
    void tearDown() { tearDownBase(); }

    @Test
    void updateSubtaskDoesNotDuplicateInPrioritized() throws Exception {
        // 1) создаём эпик
        Epic e = new Epic("E", ""); e.setStatus(Status.NEW);
        ...
        long occurrences = JsonParser.parseString(pr.body()).getAsJsonArray().asList().stream()
